...
export default class TrainingsList extends React.PureComponent<void, TrainingsListState> {
    ...
    try {
      ...
      if (storedStateJSON !== null){
        decode(
          ...
          t.interface({
            currentTraining: t.union([I.TOngoingTraining, I.TNotStartedTraining, I.TFinishedTraining, t.null]),
  ...
  updateCurrentTraining = (currentTraining: I.OngoingTraining | I.NotStartedTraining | I.FinishedTraining, editingTrainingIndex: number | null) => {
    this.setState({
      currentTraining,
    ...
    if (currentTraining) {
      switch (currentTraining.kind) {
        case 'OngoingTraining':
          const newFinishedTraining: I.FinishedTraining = {
          ...
          this.setState({
            currentTraining: null,
            finishedTrainings: newFinishedTraining.completedExercises.length > 0 ? [newFinishedTraining, ...finishedTrainings] : finishedTrainings,
          }, () => this.storeData());
          break;
        case 'FinishedTraining':
          this.setState({
            currentTraining: null,
            editingTrainingIndex: null,
            finishedTrainings: currentTraining.completedExercises.length > 0
